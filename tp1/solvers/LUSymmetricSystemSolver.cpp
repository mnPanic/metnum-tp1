...
namespace PLU{
	...
	class Matrix{
		public:
			...
			Matrix(int _rows,int _cols){
				rows = _rows;
				cols = _cols;
				cells = vector<vector<float>>(rows,vector<float>(cols,0));
			}
	...
	class System{
		public:
		System(){}
		System(int rows) : LU(rows,rows),b(rows,0.0f),P(rows,0){}
		vector<int>P;
		Matrix LU;
		vector<float>b;
	...
	{
		for(int y = 0;y < s.LU.rows;y++){
			for(int x = 0;x < s.LU.cols;x++){
				os << s.LU.cells[y][x];
				if(x<s.LU.cols-1)os << " ";
			}
			os << " | " << s.b[y] << endl;
	...
	void gaussian_elimination(System& s){
		...
		for(int i = 1;i < m.rows;i++){
			for(int r = i;r < m.rows;r++){
				...
				float coeff = m.cells[i-1][r]/m.cells[i-1][i-1];
	...
	System generate_system(string filename){
		...
		if (myfile.is_open()){
			...
			for(int i = 0;i < games;i++){
				...
				teamI-=1;
				teamJ-=1;
				m.cells[teamI][teamI] += 1.0f;
				m.cells[teamJ][teamJ] += 1.0f;
				m.cells[teamI][teamJ] -= 1.0f;
				m.cells[teamJ][teamI] -= 1.0f;
...
int main(int argc, char const *argv[]){
	...
	stopwatch.Start(0);
	PLU::System s = PLU::generate_system(input_file);
